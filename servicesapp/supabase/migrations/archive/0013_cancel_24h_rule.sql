-- ==============================================================
-- supabase/migrations/0013_cancel_24h_rule.sql
-- Add 24h cancellation rule to cancel_job.
--
-- The 24h rule has always existed for propose_reschedule but was
-- never copied to cancel_job.  Gap found during Fase 10 verification
-- (2026-06-25).
--
-- Rule: a confirmed job with a fixed confirmed_date cannot be cancelled
-- if fewer than 24 hours remain before that date.  Applies to both
-- client and worker symmetrically.  Flexible-date confirmed jobs
-- (confirmed_date IS NULL) are exempt, matching propose_reschedule's
-- existing behaviour.
--
-- Signature unchanged: cancel_job(uuid, text, text DEFAULT NULL) RETURNS uuid
-- Full body reproduced from 0009 (current live version).
-- ==============================================================
